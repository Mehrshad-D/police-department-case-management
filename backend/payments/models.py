...
class BailPayment(models.Model):
    ...
    STATUS_SUPERVISOR_APPROVAL_NEEDED = 'supervisor_approval_needed'
    ...
    gateway_reference = models.CharField(max_length=255, blank=True, unique=True)
    ...
    def save(self, *args, **kwargs):
        if not self.gateway_reference:
            self.gateway_reference = f"BAIL-{uuid.uuid4().hex[:12].upper()}"
...
class FinePayment(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.gateway_reference:
            self.gateway_reference = f"FINE-{uuid.uuid4().hex[:12].upper()}"
        super().save(*args, **kwargs)
