...
class BailPaymentSupervisorApproveView(APIView):
    ...
    def post(self, request, pk):
        ...
        bail.save()
        log_audit(request.user, 'approve', 'BailPayment', bail.pk, 'Approved')
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            if payment_type == 'bail':
                payment = BailPayment.objects.get(pk=payment_id)
                if status_param == 'success':
                    payment.status = BailPayment.STATUS_COMPLETED
                    payment.gateway_reference = ref
                    context['success'] = True
                else:
                    payment.status = BailPayment.STATUS_FAILED
                payment.save()
            elif payment_type == 'fine':
                payment = FinePayment.objects.get(pk=payment_id)
                if status_param == 'success':
        ...
        return render(request, 'payment_callback.html', context)
