...
class SuspectSupervisorReviewView(APIView):
    ...
    def post(self, request, pk):
        ...
        if action == 'reject':
            suspect.delete()
            log_audit(request.user, 'reject', 'Suspect', pk, 'Rejected')
            return Response({'success': True, 'data': {'message': 'Rejected'}})
        ...
        if suspect.case.assigned_detective_id:
            notify(suspect.case.assigned_detective, 'Approved', f'{suspect.case_id}', 'suspect_approved', 'Suspect', suspect.pk)
...
class SuspectHighPriorityListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return sorted(qs, key=lambda x: x.ranking_score, reverse=True)
